...
//      if ($announce!=$BASEURL."/announce.php")
    ...
    if (in_array($announce,$TRACKER_ANNOUNCEURLS)){
        ...
        $file_info_hash = torrent_info($_FILES["torrent"]["tmp_name"])->hash_info();
        $query = "INSERT INTO {$TABLE_PREFIX}files (info_hash, filename, url, info, category, data, size, comment, uploader,anonymous, bin_hash, file_info_hash) VALUES (\"$hash\", \"$filename\", \"$url\", \"$info\",0 + $categoria,NOW(), \"$size\", \"$comment\",$curuid, '{$anonyme}',0x$hash, '{$file_info_hash}')";
    } else
          {
          // maybe we find our announce in announce list??
            ...
            if ($internal) {
                ...
                $query = "INSERT INTO {$TABLE_PREFIX}files (info_hash, filename, url, info, category, data, size, comment, external, announce_url, uploader, anonymous, bin_hash, file_info_hash) VALUES ('{$hash}', '{$filename}', '{$url}', '{$info}', 0 + {$categoria}, NOW(), '{$size}', '{$comment}', 'yes', '{$announce}', {$curuid}, '{$anonyme}', '0x{$hash}', '{$file_info_hash}')";
            }
        }
